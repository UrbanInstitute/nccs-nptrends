...
# (1) - Extract Data
nptrends_y5_raw <- data.table::fread("Y:/CNP/Generosity Commission/Year 5/Restricted Dataset/RESTRICTED_Y5.csv")
...
nptrends_y5 <- nptrends_y5_raw |>
  dplyr::mutate(
    ...
    National = "National") %>% 
  #Removes rows with 97 or 98 before finding average
  dplyr::filter(!if_any(
    .cols = c(
      matches("^(ProgDem_|FinanceChng_|PrgSrv_|PercentDem_|Dem_)"),
              "CARES_Rcv","FndRaise_DAF_Grnt_Chng",
              "FndRaise_Priv_Grnt_Chng", "FndRaise_Corp_Grnt_Chng",
              "PrgSrvc_Amt_Fee","FndRaise_TotExp"
    ), 
    ~ .x %in% c(97,98)
    ))%>% 
  ...
  dplyr::mutate(
    ...
    FndRaise_GovFunding = binary_flag(
      data = .,FndRaise_LocGvtGrnt_Rcv, FndRaise_StateGvtGrnt_Rcv, 
    ...
    Reserves_Est = dplyr::case_when(
      Reserves_Est == NA & 
        Reserves_NA_X == 1 ~ 0,
      .default = Reserves_Est
    ),
    #Each organization needs to divide Reserves by TotExp
    Cash_Reserves = Reserves_Est/TotExp,
  
    Staff_RegVlntr_2024 = dplyr::case_when(
      Staff_RegVlntr_2024 == NA & 
        Staff_RegVlntr_NA == 1 ~ 0,
      .default = Staff_RegVlntr_2024
    ),
    Staff_EpsdVltnr_2024 = dplyr::case_when(
      Staff_EpsdVltnr_2024 == NA & 
        Staff_EpsdVltnr_NA == 1 ~ 0,
      .default = Staff_EpsdVltnr_2024
    ),
    Staff_Boardmmbr_2024 = dplyr::case_when(
      Staff_Boardmmbr_2024 == NA & 
    ...
    Staff_Fulltime_2024= dplyr::case_when(
      Staff_Fulltime_2024 == 0 ~ "0",
      Staff_Fulltime_2024 == 1 ~ "1",
      Staff_Fulltime_2024 >= 2 &  Staff_Fulltime_2024 <= 9 ~ "2-9",
      Staff_Fulltime_2024 >= 10 & Staff_Fulltime_2024 <= 49 ~ "10-49",
      Staff_Fulltime_2024 >= 50 ~ "50+",
      Staff_Fulltime_NA == 1 & is.na(Staff_Fulltime_2024) ~ "0",
      .default = NA_character_
    ),
    Staff_Parttime_2024 = dplyr::case_when(
      Staff_Parttime_2024 == 0 ~ "0",
      Staff_Parttime_2024 == 1 ~ "1",
      Staff_Parttime_2024 >= 2 &  Staff_Parttime_2024 <= 9 ~ "2-9",
      Staff_Parttime_2024 >= 10 & Staff_Parttime_2024 <= 49 ~ "10-49",
      Staff_Parttime_2024 >= 50 ~ "50+",
      Staff_Parttime_NA == 1 & is.na(Staff_Parttime_2024) ~ "0",
      .default = NA_character_
    ),
...
metrics <- list(
  ...
  FndRaise_GovFunding = "% of respondents",
  ...
  Staff_RegVlntr_2024 = "median",
  Staff_EpsdVltnr_2024 = "median",
  ...
  Staff_Fulltime_2024 = "% of respondents",
  Staff_Parttime_2024 = "% of respondents",
