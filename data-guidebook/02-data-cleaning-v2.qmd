...
setwd( "C:/Users/jdlec/Dropbox (Personal)/00 - URBAN/np-trends/DATA" )
...
weighted_survey_df <- haven::read_dta( "DATA-PREP/02-data-intermediate/01-wave-one/wave-01-data-intermediate.dta")
...
# CONVERT RULE STRING TO CASE_MATCH FUNCTION
...
parse_rules <- function(x) {
  ...
  L <- lapply( L, function(x){ gsub( "[']{2,}","'",x)} )
  
  R <- sapply( L, function(x){paste0(x,collapse=" ~ ")}, USE.NAMES=F )
  R <- paste0( R, collapse=", " )
  
  STRING1  <- "dplyr::case_match(."
  STRING2  <- ".default = NA )"
  rule     <- paste0( c( STRING1, R, STRING2 ), collapse=", " )
...
# CONVERT DF TO SURVEY OBJECT WIT MEMISC
...
# APPLY RECODING RULES TO COLUMNS AND ADD LABELS 
recode <- function( df, 
                    RULES=RULES,
                    COLUMNS=COLUMNS, 
                    VALUES=VALUES, 
...
boolean.columns <- tidyselect::all_of( bool_qns )

survey_df[ boolean.columns ] |> 
  lapply( table ) |>
  pander::pander()
```
...
codebook( survey_recode_df[ bool_qns ] )
```
...
codebook( survey_recode_df[ fundraise_skrcv_qns_bool ] )
...
# setwd("Y:/CNP/Generosity Commission/")
setwd( "C:/Users/jdlec/Dropbox (Personal)/00 - URBAN/np-trends/DATA" )
year1_raw <- readr::read_csv("wave-01-data-intermediate.csv")
```
...
```{r}
write.csv( survey_recode_df, "TEST-LABELS-V2.csv" )
```

```{r}
# setwd( "Y:/CNP/Generosity Commission/DATA-PREP/02-data-intermediate/02-wave-two/" )

setwd( "C:/Users/jdlec/Dropbox (Personal)/00 - URBAN/np-trends/DATA" )

memisc::Write( codebook(survey_recode_df),    file="DATA-CODEBOOK.txt" )

write.csv( survey_recode_df, "WAVE-02-DATA-V2-WITH-LABELS.csv" )
```