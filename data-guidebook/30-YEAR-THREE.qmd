...
Standardize variable names using Year2 names: 
...
torename <- 
  crosswalk %>% 
  dplyr::select( vname_y2, vname_y3 ) %>% 
  tidyr::drop_na()

survey_df <- 
  survey_df %>% 
  dplyr::rename_at(
     vars( torename$vname_y3 ), 
     ~ torename$vname_y2 )
```


```{r}
# torename$vname_y3 %>% setdiff( names(survey_df) )
torename %>% pander::pander()
```


```{r}
dd$vname_raw %>% setdiff( names(survey_df) )
names(survey_df)  %>% setdiff( dd$vname_raw )
```
