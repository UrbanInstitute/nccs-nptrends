...
The f_address column now has parsed addresses that can be formatted using a geocoder of choice. We can now rejoin the parsed address data via EINs with our original data set.

```{r}
survey_geoproc_df <- survey_recode_df %>% 
  dplyr::filter(! is.na(EIN)) %>% 
  tidylog::left_join(postmastr_faddress_parsed, by = "EIN")
```
