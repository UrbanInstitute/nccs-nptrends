...
volimportance <- survey_qns[48]
donimportance <- survey_qns[49]

# Fundraising
fundraise_qns <- survey_qns[50:60]
fundraise_numeric <- survey_qns[50:59]
fundraise_text <- survey_qns[60]

# Amount for major gifts
majorgift_qn <- survey_qns[61:61]

# Replicate approach if above is ok with Jesse
...
survey_recode_df <- survey_df %>%
  dplyr::mutate(
    dplyr::across(
      c(program_change_qns_numeric, fundraise_numeric),
      ~ dplyr::case_match(.,
                          "Yes" ~ 1, "No" ~ 0, "N/A" ~ 99,
                          "Unsure" ~ 98, "-99" ~ -99,
                          .default = NA)
    ...
    dplyr::across(
      c(staff_qns_text, program_change_qns_text, fundraise_text),
      ~ dplyr::case_match(.,
                          "-99" ~ "-99", NA ~ NA,
